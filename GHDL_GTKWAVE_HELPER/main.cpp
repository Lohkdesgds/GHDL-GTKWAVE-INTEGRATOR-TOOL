		...
		try {
			...
			if (in.find("gtkw") == 0) {
...

				{
					...
					if (err) { cout << console::color::RED << "# (ABORT) Error setting up main path: " << err.message(); continue; }
				}

				cout << console::color::YELLOW << "# Downloading package...";

				if (!download_to(download_path, envpath + mytmpfile)) { cout << console::color::RED << "# (ABORT) Error downloading package. Please try again later or contact developer!"; continue; }
				...
				{
					cout << console::color::RED << "# (ABORT) Error extracting package. Please try again later or contact developer!"; continue;
				}
